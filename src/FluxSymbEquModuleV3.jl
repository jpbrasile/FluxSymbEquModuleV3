module FluxSymbEquModuleV3 
export SolveSymbolic, NumericSolve, PlotSol
#export SolveSymbolic(myInputCircuit), NumericSolve(myStrgToSolve1,myLoop), PlotSol(sol,myVrArray)	
...
function flatEqs(sympyEq,myVar)
	...
	for j in 1:length(myVar)
		for i in 1: length(myVar[j])
			try
	eval(Meta.parse(string(myVar[j][i]) *"=[]"))
	eval(Meta.parse("@variables "*string(myVar[j][i])))
			catch
			end
		end
	end

eqs=eval(Meta.parse("["*replace(flattenEqs1*",",","=>"~0,")[1:end-1]*"]"))
	vars=eval(Meta.parse("["*flatVars(myVar)*"]"))
	
	Az=simplify.(Symbolics.solve_for(eqs,vars;simplify=false))
...
mysol=string.(Az)
	...
	excitVar1=fillCmpt(LLCExample)[3]
	#replace.(rect1,excitVar=>excitVar*"*p[1]")
	myStrgToSolve=flatEqs(replace.(rect1,excitVar1[1]=>excitVar1[1]*"*p[1]"),myVar)[1]*"; end"
	myStrgToSolve1="function eqDiffToSolve!(du,u,p,t) "* myst *" = u; " *replace(myStrgToSolve,","=>"; ") #!!!!!!
	#eval(Meta.parse(myStrgToSolve1))
   myStrgToSolve1=replace(myStrgToSolve1, "-*"=>"-");
...
function NumericSolve(myStrgToSolve1,myLoop,tspan,freq)	
	...
	u₀=[0,0,0,0]
	p=[-1.0,1.0]
	nbPeriod=(tspan[2]-tspan[1])*freq
	dosetimes = collect(1:nbPeriod)/freq
	#eval(Meta.parse(myStrgToSolve1))
	for i in 1:length(myLoop)
	...
	function PlotSol(sol,myVrArray)
